# Pro One-Time License Plan

## 1. Goal

为 YtbDown 增加 Pro 买断授权和下载限制：

- 免费版终身只能成功下载 10 个视频。
- Pro 一次性买断，解除下载数量限制。
- 默认保留 Stripe 收款，同时支持咸鱼、微店、自建 WordPress / WooCommerce 等外部购买渠道。
- 支付成功或外部订单确认后，由自建服务端生成 license key，并通过邮件发送给用户。
- 用户无需注册账号，使用购买邮箱 + license key 激活。
- 默认每个 license 支持 3 台设备。
- 旧设备不在身边时，支持自动迁移，减少人工处理。

核心原则：支付平台只负责收款和订单确认；我们自己的 License Server 负责序列号、设备激活、离线 token、设备迁移和客户端授权判断。
...
- license 本身永久有效，但客户端使用 7 天有效期的签名 token 做离线授权。
- 退款、争议、key 泄露或后台禁用后，Pro 权限在 token 过期或刷新失败后失效。
...
- Stripe Checkout、WooCommerce Checkout、咸鱼/微店订单备注或购买表单收集的购买邮箱。
- 服务端生成的 license key。
...
除 Stripe 外，v1 增加外部订单履约能力，避免业务必须依赖 Stripe 主体资质。

支持渠道：

- 咸鱼：
  - 商品详情或自动回复中要求用户提交购买邮箱。
  - 订单确认后，由后台录入订单或导入订单，License Server 生成 license key。
  - 系统自动给购买邮箱发送激活码。
  - 站内消息自动发送激活码只有在平台提供稳定 API 或商家工具支持时才启用；否则不把站内消息作为唯一发货方式。
- 微店：
  - 商品下单时要求填写邮箱。
  - 若微店提供订单 webhook / API，则接入自动履约。
  - 若暂时没有稳定 API，则用订单导入或后台手动确认触发自动邮件发货。
- 自建 WordPress / WooCommerce：
  - 推荐作为非 Stripe 的主要自动化销售渠道。
  - WooCommerce 收款成功后通过 webhook 通知 License Server。
  - License Server 校验 webhook 签名或共享密钥后创建 license，并发送邮件。

统一要求：

- 所有渠道必须收集购买邮箱。
- 所有渠道最终都调用 License Server 的同一套 license 创建逻辑。
- `order_source` 区分 `stripe`、`xianyu`、`weidian`、`woocommerce`、`manual`。
- `external_order_id` 保存外部订单号，防止重复发货。
- 发货以邮件为主，平台站内消息为辅助。
- 不在客户端内保存任何支付平台密钥。

外部渠道履约状态：

- `pending`：订单已记录，但尚未确认可发货。
- `paid`：订单已确认支付，已创建或准备创建 license。
- `fulfilled`：license 已生成并已尝试发送邮件。
- `refunded`：订单退款，license 应被禁用或进入人工复核。
- `disputed`：订单异常或争议，license 可临时禁用。
...
2. 打开 Stripe Checkout，使用一次性付款模式。
3. 用户完成支付。
4. Stripe 发送 webhook 到 License Server。
5. License Server 验证 webhook 签名。
6. License Server 根据 Stripe customer / Checkout Session / PaymentIntent 信息创建 license。
...
License Server 是唯一持有支付平台密钥、webhook secret、邮件 API key、token signing private key 的地方。

客户端不能直接访问 Stripe、WooCommerce、咸鱼、微店等支付或订单 API，也不能内置任何服务端密钥。
...
- `plan`
- `status`: `active` / `disabled` / `refunded` / `disputed`
...
- token 过期且服务端不可用时，允许使用一次 24 小时 emergency grace。
- grace 也过期且 refresh 仍失败，降级为免费版。
...
`device_id` 和 `installation_id` 的权威存储使用系统安全存储：macOS Keychain、Windows Credential Manager。`$APP_DATA/entitlement.json` 只做缓存和 UI 状态。Rust/Tauri 侧建议使用 `keyring` crate 统一访问。
...
- 所有设备激活和迁移都由服务端决定。
- Stripe webhook 必须验签。
- WooCommerce webhook 必须验签或校验共享密钥。
- 外部订单后台履约接口必须鉴权，不能公开发 license。
- 设备迁移必须限频。
...
- 配置 Checkout。
- 配置 webhook endpoint。
- 配置 WooCommerce 商品和 webhook。
- 准备咸鱼、微店商品说明，要求买家提交购买邮箱。
- 配置邮件服务。
...
- 上线 WooCommerce 商品页。
- 发布新版本 App。
- 监控 webhook、外部订单导入、邮件投递、激活失败率、设备迁移次数。
...
- WooCommerce 支付完成后生成 license key。
- 咸鱼/微店订单手动确认或导入后生成 license key。
- 同一个外部订单号重复提交时不会生成多个 license。
...
- 服务端离线时，未过期 Pro token 继续可用。
- token 过期且服务端仍离线时，允许一次 24 小时 emergency grace。
...
- Pro 购买方式：一次性买断。
- token 有效期：7 天。
- 服务端故障 emergency grace：24 小时，单个 token 只能使用一次。
- 闲置设备自动迁移阈值：30 天未使用。
- 每 30 天最多自动迁移：3 次。
- 验证码有效期：10 分钟。
- 每 24 小时最多发送验证码：5 次。
- 每 24 小时最多成功激活：5 次。
- 免费额度计数：服务端计数为准，本地只缓存剩余额度。
- `device_id` / `installation_id`：系统安全存储为准，`$APP_DATA` 只缓存。

## 15. License Server Outage Policy

服务端离线时按用户类型区分处理：

- 已激活 Pro：
  - 本地签名 token 未过期时继续 Pro。
  - token 快过期但 refresh 失败时，提示“授权服务器暂时不可用，Pro 离线授权剩余 X 天”。
  - token 已过期且服务端仍不可用时，允许一次 24 小时 emergency grace。
  - emergency grace 也过期后，降级为免费版。
- 新激活或新购买：
  - 必须等待 License Server 恢复。
  - Stripe webhook 会重试，服务端恢复后再生成 license key 并发邮件。
  - WooCommerce webhook 或外部订单导入可在服务端恢复后重试。
  - 咸鱼/微店订单以邮件发货为主；如果平台消息无法自动发送，后台提供可复制发货文本。
- 免费版：
  - 使用本地缓存的免费剩余额度兜底。
  - 本地缓存额度用完后，必须联网同步免费额度或激活 Pro。

## 16. Linked Repositories

- 客户端公仓：`https://github.com/elsakane2015/YtbDownGUI.git`
  - 当前 Pro 开发分支：`pro-dev`
  - 职责：Tauri 客户端、下载额度展示、Pro 激活 UI、本地 token 验签、下载入队限制。
- 服务端私仓：`https://github.com/elsakane2015/ytbdown-license-server.git`
  - 默认分支：`main`
  - 职责：Stripe webhook、WooCommerce webhook、外部订单履约、license key 生成、邮件发送、设备激活、token 签发、免费额度服务端计数。

两个仓库通过 License Server API 和签名 token 联动。客户端不保存支付平台密钥、邮件 API key 或 token 签名私钥。
